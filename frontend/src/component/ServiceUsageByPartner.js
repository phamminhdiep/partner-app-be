...
function ServiceUsageByPartner() {
    ...
    const handleSubmit = async () => {
        ...
        let amount = selectedServices.reduce((total, serviceId) => {
            const service = dataList.find(data => data.id === serviceId);
            return total + service.total;
        }, 0);

        let serviceUsedList = selectedServices.toString();
        console.log('123serviceUsedList: ' + serviceUsedList);
        ...
        try {
            ...
            const response = await axios.get(
                `http://localhost:8080/api/v1/payment/create-payment`,
                {
                    params: {
                        amount: amount,
                        serviceUsedList: serviceUsedList
                    },
                    headers: {
                        'Content-Type': 'application/json'
                    }
